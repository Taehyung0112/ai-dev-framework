...
環境變數:
  ANTHROPIC_API_KEY    (必填) Claude 自動摘要 commit
  NOCODB_XC_TOKEN      (選填) 覆蓋內建預設 token
  GIT_REPOS            (選填) 多個 repo 路徑，用分號分隔
"""
...
if not _people_id_env:
    print("[ERROR] 請先設定環境變數 NOCODB_PEOPLE_ID（你在 NocoDB 的 People ID）")
    print("        執行 /weekly-report skill 查看設定說明")
    sys.exit(1)
PEOPLE_ID = int(_people_id_env)

DEFAULT_REPOS = [r"C:\Users\115016\incubator"]
...
def main() -> None:
    ...
    # 1. Collect commits
    repos       = [r.strip() for r in os.environ.get("GIT_REPOS", "").split(";") if r.strip()] \
                  or DEFAULT_REPOS
